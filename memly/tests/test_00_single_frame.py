...
np.testing.assert_allclose(np.round(memly.membrane.get_lipid_vector(x.sim[0],
                                                                    ...
                                                                    0),
                                    3),
                           np.asarray([0.003,  0.486, -1.064]))

assert len(x.raw_leaflets) == len(x.sim)
assert len(x.raw_leaflets[0][0]) == 167
assert np.max(list(x.raw_leaflets[0][0])) == 199
assert np.median(list(x.raw_leaflets[0][0])) == 84.0
assert np.min(list(x.raw_leaflets[0][0])) == 0

assert len(x.leaflets) == len(x.sim)
assert len(x.leaflets[0]["upper"]) == 167
assert np.max(x.leaflets[0]["upper"]) == 199
assert np.median(x.leaflets[0]["upper"]) == 84.0
